...
dataset = SmokeDataset(r"data/dataset_v1/test",None,transform,15)
batch_size = 1
...
model = CNNLSTM()
model.load_state_dict(torch.load("./model/CNNLSTM/save/model_best.pth"))
model.to(device)
model.eval()
# smoke: 1
# nonsmoke: 0
...
with torch.no_grad():
    ...
    tp = tn = fp = fn = 0
    for image_paths, x, y in test_loader:
